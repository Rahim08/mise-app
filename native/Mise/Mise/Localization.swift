...
let STRINGS: [String: [Lang: String]] = [
    ...
    "mg.topups": tr("Top-ups", "Поступления", "Entrate", "Entrées", "Daxilolmalar", "Girişler", "Надходження", "Түсімдер"),
    "mg.addTx": tr("Add transaction", "Добавить транзакцию", "Aggiungi transazione", "Ajouter une transaction", "Əməliyyat əlavə et", "İşlem ekle", "Додати транзакцію", "Транзакция қосу"),
    "mg.topupNew": tr("New top-up", "Новое поступление", "Nuova entrata", "Nouvelle entrée", "Yeni daxilolma", "Yeni giriş", "Нове надходження", "Жаңа түсім"),
    "mg.topupEdit": tr("Edit top-up", "Изменить поступление", "Modifica entrata", "Modifier l’entrée", "Daxilolmanı dəyiş", "Girişi düzenle", "Змінити надходження", "Түсімді өзгерту"),
    "mg.topupDate": tr("Date", "Дата", "Data", "Date", "Tarix", "Tarih", "Дата", "Күні"),
    "mg.topupAmount": tr("Amount", "Сумма", "Importo", "Montant", "Məbləğ", "Tutar", "Сума", "Сома"),
    "mg.topupReason": tr("Reason", "Причина", "Motivo", "Motif", "Səbəb", "Sebep", "Причина", "Себеп"),
    "mg.topupHint": tr("Added to the collection balance. The cash register is not affected.", "Добавляется в баланс инкассации. Касса не затрагивается.", "Aggiunto al saldo del versamento. La cassa non viene toccata.", "Ajouté au solde d’encaissement. La caisse n’est pas affectée.", "İnkassasiya balansına əlavə olunur. Kassa dəyişmir.", "Kasa devri bakiyesine eklenir. Kasa etkilenmez.", "Додається до балансу інкасації. Каса не змінюється.", "Инкассация балансына қосылады. Касса өзгермейді."),
    "mg.topupNone": tr("No top-ups yet", "Поступлений пока нет", "Nessuna entrata", "Aucune entrée", "Hələ daxilolma yoxdur", "Henüz giriş yok", "Надходжень поки немає", "Түсімдер әлі жоқ"),
    "mg.topupErrAmount": tr("Enter an amount greater than 0", "Введите сумму больше 0", "Inserisci un importo maggiore di 0", "Saisissez un montant supérieur à 0", "0-dan böyük məbləğ daxil edin", "0’dan büyük bir tutar girin", "Введіть суму більше 0", "0-ден үлкен соманы енгізіңіз"),
    "mg.topupErrReason": tr("Reason is required", "Укажите причину", "Il motivo è obbligatorio", "Le motif est obligatoire", "Səbəb tələb olunur", "Sebep gerekli", "Вкажіть причину", "Себебін көрсетіңіз"),
    "mg.topupErrDate": tr("Date cannot be in the future", "Дата не может быть в будущем", "La data non può essere nel futuro", "La date ne peut pas être dans le futur", "Tarix gələcəkdə ola bilməz", "Tarih gelecekte olamaz", "Дата не може бути в майбутньому", "Күн болашақта болмауы керек"),
    "mg.topupDeleteSure": tr("Tap again to delete", "Нажмите ещё раз для удаления", "Tocca di nuovo per eliminare", "Appuyez encore pour supprimer", "Silmək üçün yenidən toxunun", "Silmek için tekrar dokunun", "Натисніть ще раз для видалення", "Жою үшін қайта басыңыз"),
    "an.topup": tr("Top-up", "Поступление", "Entrata", "Entrée", "Daxilolma", "Giriş", "Надходження", "Түсім"),
    "mg.topupErrNegative": tr("Can’t reduce or delete: collection balance would go below zero. Removable amount: {avail}", "Нельзя уменьшить или удалить: баланс инкассации уйдёт в минус. Можно убрать не больше: {avail}", "Impossibile ridurre o eliminare: il saldo del versamento andrebbe sotto zero. Importo rimovibile: {avail}", "Impossible de réduire ou supprimer : le solde d’encaissement passerait sous zéro. Montant retirable : {avail}", "Azaltmaq və ya silmək olmaz: inkassasiya balansı sıfırdan aşağı düşəcək. Çıxarıla bilən məbləğ: {avail}", "Azaltılamaz veya silinemez: kasa devri bakiyesi sıfırın altına düşer. Çıkarılabilir tutar: {avail}", "Не можна зменшити або видалити: баланс інкасації піде в мінус. Можна прибрати не більше: {avail}", "Азайтуға немесе жоюға болмайды: инкассация балансы нөлден төмен түседі. Алуға болатын сома: {avail}"),
    "mg.topupErrVerify": tr("Could not verify the collection balance. Try again.", "Не удалось проверить баланс инкассации. Повторите.", "Impossibile verificare il saldo del versamento. Riprova.", "Impossible de vérifier le solde d’encaissement. Réessayez.", "İnkassasiya balansını yoxlamaq mümkün olmadı. Yenidən cəhd edin.", "Kasa devri bakiyesi doğrulanamadı. Tekrar deneyin.", "Не вдалося перевірити баланс інкасації. Повторіть.", "Инкассация балансын тексеру мүмкін болмады. Қайталаңыз."),
    ...
    "an.debtsLoadFailed": tr("Couldn't check for updates — the number may be outdated", "Не удалось проверить обновления — сумма может быть устаревшей", "Impossibile controllare gli aggiornamenti — l'importo potrebbe essere obsoleto", "Impossible de vérifier les mises à jour — le montant peut être obsolète", "Yeniləmələr yoxlanıla bilmədi — məbləğ köhnəlmiş ola bilər", "Güncellemeler kontrol edilemedi — tutar güncel olmayabilir", "Не вдалося перевірити оновлення — сума може бути застарілою", "Жаңартуларды тексеру мүмкін болмады — сома ескірген болуы мүмкін"),
    ...
    "pe.salaryWord":     tr("Salary", "ЗП", "Stipendio", "Salaire", "Maaş", "Maaş", "ЗП", "ЖА"),
    "pe.markAsDebt":     tr("Mark as debt (no cash now)", "Отметить как долг (нет налички)", "Segna come debito (contanti non disponibili)", "Marquer comme dette (pas de liquide)", "Borc kimi qeyd et (nağd pul yoxdur)", "Borç olarak işaretle (nakit yok)", "Позначити як борг (немає готівки)", "Борыш деп белгілеу (қолма-қол жоқ)"),
    "pe.markedAsDebt":   tr("Recorded as debt", "Записано в долги", "Registrato come debito", "Enregistré comme dette", "Borc kimi qeydə alındı", "Borç olarak kaydedildi", "Записано в борги", "Борыш ретінде жазылды"),
    "pe.raceRetry":      tr("Someone else was saving at the same moment — try again", "Кто-то ещё сохранял в этот же момент — попробуйте ещё раз", "Qualcun altro stava salvando nello stesso momento — riprova", "Quelqu’un d’autre enregistrait au même moment — réessayez", "Eyni anda başqası yadda saxlayırdı — yenidən cəhd edin", "Aynı anda başka biri kaydediyordu — tekrar deneyin", "Хтось інший зберігав у цей самий момент — спробуйте ще раз", "Дәл осы сәтте басқа біреу сақтап жатты — қайта көріңіз"),
    ...
    "pe.paymentRollbackFailed": tr("Payment state may be inconsistent — check with the owner before paying again", "Возможна нестыковка по выплате — сверьтесь с владельцем перед повторной оплатой", "Lo stato del pagamento potrebbe essere incoerente — verifica con il titolare prima di ripagare", "L’état du paiement peut être incohérent — vérifiez avec le propriétaire avant de repayer", "Ödəniş vəziyyəti uyğunsuz ola bilər — yenidən ödəmədən əvvəl sahibkarla yoxlayın", "Ödeme durumu tutarsız olabilir — tekrar ödemeden önce işletme sahibiyle kontrol edin", "Стан виплати може бути неузгодженим — звірте з власником перед повторною оплатою", "Төлем күйі сәйкессіз болуы мүмкін — қайта төлемес бұрын иесімен тексеріңіз"),
