...
let STRINGS: [String: [Lang: String]] = [
    ...
    "pe.swapRequestTitle": tr("Swap request", "Запрос на обмен", "Richiesta di cambio", "Demande d’échange", "Növbə dəyişmə sorğusu", "Vardiya değişim talebi", "Запит на обмін", "Ауысым сұранысы"),
    "pe.swapRequestBody":  tr("{name} is offering a shift swap", "{name} предлагает обмен сменой", "{name} propone uno scambio di turno", "{name} propose un échange de service", "{name} növbə dəyişməyi təklif edir", "{name} vardiya değişimi öneriyor", "{name} пропонує обмін зміною", "{name} ауысым алмасуды ұсынады"),
    ...
    "pe.notifications":  tr("Notifications", "Уведомления", "Notifiche", "Notifications", "Bildirişlər", "Bildirimler", "Сповіщення", "Хабарламалар"),
    "pe.noNotifs":       tr("Quiet for now — no notifications", "Пока тихо — уведомлений нет", "Tutto tranquillo — nessuna notifica", "Rien pour l’instant — aucune notification", "Hələ sakitlikdir — bildiriş yoxdur", "Şimdilik sessiz — bildirim yok", "Поки тихо — сповіщень немає", "Әзірге тыныш — хабарлама жоқ"),
    ...
    "pe.presetTemplates": tr("Add ready-made templates", "Добавить готовые шаблоны", "Aggiungi modelli pronti", "Ajouter des modèles prêts", "Hazır şablonlar əlavə et", "Hazır şablonlar ekle", "Додати готові шаблони", "Дайын үлгілерді қосу"),
    ...
    "pe.presetOpenHall1": tr("Turn on lights and music", "Включить свет и музыку", "Accendi luci e musica", "Allumer lumières et musique", "İşıq və musiqini yandır", "Işık ve müziği aç", "Увімкнути світло і музику", "Жарық пен әуенді қосу"),
    "pe.presetOpenHall2": tr("Check the register", "Проверить кассу", "Controlla la cassa", "Vérifier la caisse", "Kassanı yoxla", "Kasayı kontrol et", "Перевірити касу", "Кассаны тексеру"),
    "pe.presetOpenHall3": tr("Wipe the tables", "Протереть столы", "Pulisci i tavoli", "Essuyer les tables", "Masaları sil", "Masaları sil", "Протерти столи", "Үстелдерді сүрту"),
    "pe.presetCloseHall1": tr("Close out the register", "Сдать кассу", "Chiudi la cassa", "Clôturer la caisse", "Kassanı təhvil ver", "Kasayı kapat", "Здати касу", "Кассаны тапсыру"),
    "pe.presetCloseHall2": tr("Turn off the equipment", "Выключить оборудование", "Spegni le attrezzature", "Éteindre les équipements", "Avadanlığı söndür", "Ekipmanları kapat", "Вимкнути обладнання", "Жабдықты өшіру"),
    "pe.presetCloseHall3": tr("Close the hall", "Закрыть зал", "Chiudi la sala", "Fermer la salle", "Zalı bağla", "Salonu kapat", "Закрити зал", "Залды жабу"),
    "pe.presetOpenBar1": tr("Check the ice", "Проверить лёд", "Controlla il ghiaccio", "Vérifier la glace", "Buzu yoxla", "Buzu kontrol et", "Перевірити лід", "Мұзды тексеру"),
    "pe.presetOpenBar2": tr("Fill the coffee machine", "Заправить кофемашину", "Rifornisci la macchina del caffè", "Remplir la machine à café", "Kofe maşınını doldur", "Kahve makinesini doldur", "Заправити кавомашину", "Кофе машинасын толтыру"),
    "pe.presetSanitationTitle": tr("Sanitation audit", "Санитарный аудит", "Audit igienico-sanitario", "Audit sanitaire", "Sanitar audit", "Sağlık denetimi", "Санітарний аудит", "Санитарлық аудит"),
    "pe.presetSanitation1": tr("Hands washed and gloves on", "Руки вымыты, перчатки надеты", "Mani lavate, guanti indossati", "Mains lavées, gants portés", "Əllər yuyulub, əlcək taxılıb", "Eller yıkandı, eldiven takıldı", "Руки вимиті, рукавички одягнені", "Қол жуылды, қолғап киілді"),
    "pe.presetSanitation2": tr("Fridge temperature within range", "Температура холодильника в норме", "Temperatura frigo nella norma", "Température du frigo correcte", "Soyuducunun temperaturu normadadır", "Buzdolabı sıcaklığı normalde", "Температура холодильника в нормі", "Тоңазытқыш температурасы нормада"),
    "pe.presetSanitation3": tr("No expired products", "Нет просроченных продуктов", "Nessun prodotto scaduto", "Aucun produit périmé", "Vaxtı keçmiş məhsul yoxdur", "Süresi geçmiş ürün yok", "Немає прострочених продуктів", "Мерзімі өткен өнім жоқ"),
    "pe.presetSanitation4": tr("Trash bins closed and clean", "Мусорные баки закрыты и чисты", "Bidoni chiusi e puliti", "Poubelles fermées et propres", "Zibil qutuları bağlı və təmizdir", "Çöp kovaları kapalı ve temiz", "Сміттєві баки закриті й чисті", "Қоқыс жәшіктері жабық және таза"),
    ...
    "ns.lowStock":        tr("Low stock alerts", "Остатки заканчиваются", "Scorte in esaurimento", "Stock bas", "Stok azalması", "Stok uyarısı", "Сповіщення про запаси", "Қор азаю хабарламалары"),
    ...
    "ns.voiceTasks":      tr("Voice task confirmation", "Подтверждение голосовой задачи", "Conferma compito vocale", "Confirmation tâche vocale", "Səsli tapşırıq təsdiqi", "Sesli görev onayı", "Підтвердження голосового завдання", "Дауыстық тапсырманы растау"),
    // Shift reminder
