...
private let NOTIFY: [String: [String]] = [
    ...
    "notify.newTaskTitle":   ["New task", "Новая задача", "Nuovo compito", "Nouvelle tâche", "Yeni tapşırıq", "Yeni görev", "Нове завдання", "Жаңа тапсырма"],

    "notify.swapRequestTitle":  ["Swap request", "Запрос на обмен", "Richiesta di cambio", "Demande d’échange", "Növbə dəyişmə sorğusu", "Vardiya değişim talebi", "Запит на обмін", "Ауысым сұранысы"],
    "notify.swapRequestBody":   ["{name} is offering a shift swap {date}", "{name} предлагает обмен сменой {date}", "{name} propone uno scambio di turno {date}", "{name} propose un échange de service {date}", "{name} növbə dəyişməyi təklif edir {date}", "{name} vardiya değişimi öneriyor {date}", "{name} пропонує обмін зміною {date}", "{name} ауысым алмасуды ұсынады {date}"],
    ...
    "notify.swapPendingBody":   ["Colleague accepted the swap — needs manager approval", "Коллега принял обмен — нужно одобрение менеджера", "Il collega ha accettato lo scambio — serve l’approvazione del manager", "Le collègue a accepté l’échange — approbation du manager requise", "Həmkar dəyişimi qəbul etdi — menecer təsdiqi lazımdır", "Meslektaş takası kabul etti — yönetici onayı gerekiyor", "Колега прийняв обмін — потрібне схвалення менеджера", "Әріптес алмасуды қабылдады — менеджер мақұлдауы қажет"],
    ...
    "notify.swapDeclinedByPeerBody":    ["Colleague declined the swap request", "Коллега отклонил запрос на обмен", "Il collega ha rifiutato la richiesta di scambio", "Le collègue a refusé la demande d’échange", "Həmkar dəyişim sorğusunu rədd etdi", "Meslektaş takas talebini reddetti", "Колега відхилив запит на обмін", "Әріптес алмасу сұранысын қабылдамады"],
    "notify.swapDeclinedByManagerBody": ["Manager declined the swap", "Менеджер отклонил обмен", "Il manager ha rifiutato lo scambio", "Le manager a refusé l’échange", "Menecer dəyişimi rədd etdi", "Yönetici takası reddetti", "Менеджер відхилив обмін", "Менеджер алмасуды қабылдамады"],
    ...
    "notify.catHousehold": ["Household", "Хозтовары", "Casalinghi", "Ménage", "Təsərrüfat", "Ev", "Господарські", "Шаруашылық"],
    ...
    "notify.trialEndingBody":  ["Trial ends in {n} d.", "Триал заканчивается через {n} дн.", "La prova termina tra {n} g.", "L’essai se termine dans {n} j.", "Sınaq {n} gündən sonra bitir", "Deneme {n} gün içinde bitiyor", "Пробний період закінчується через {n} дн.", "Сынақ {n} күннен кейін аяқталады"],

    "notify.bookingTitle": ["New booking", "Новая бронь", "Nuova prenotazione", "Nouvelle réservation", "Yeni rezerv", "Yeni rezervasyon", "Нове бронювання", "Жаңа брондау"],
    "notify.bookingTable": ["Table", "Стол", "Tavolo", "Table", "Masa", "Masa", "Стіл", "Үстел"],
    "notify.lowStockTitle": ["Low stock alert", "Товар заканчивается", "Scorte in esaurimento", "Stock bas", "Stok azalması", "Stok uyarısı", "Сповіщення про запаси", "Қор азаю ескертуі"],
