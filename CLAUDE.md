...
Б6 — скоринг аудитов с весами (по запросу клиентов): код готов веб + iOS (19.07), не закоммичено. Вес пункта (1–9) редактируется только для разовых аудитов (kind='audit'); чек-листы смены весов не задают. Формула: % от суммы весов, N/A вне знаменателя. Попутно починен баг в `PeopleModel.saveChecklistTemplate` — вес пункта терялся при сохранении шаблона.

Открыто:
1. Ручное (юзер): смоук мобильного таб-бара дашборда (В1), пересборка iOS после распила People + Б6 (заодно фикс `{date}` в пуше открытия/закрытия кассы — версио-скью между старым iOS-билдом и новым сервером, см. коммит `35dae62`), тест-checkout биллинга → прогон `scripts/stripe-setup.mjs` с sk_live
2. Sprint 2: кастомизация Mise Menu, скриншоты/локализация App Store
